...
func (m *Repository) Reservation(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		m.App.Session.Put(r.Context(), "error", errors.New("could not get reservation from session"))
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	room, err := m.DB.GetRoomByID(res.RoomID)
	if err != nil {
		m.App.Session.Put(r.Context(), "error", errors.New("can't find room"))
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		m.App.Session.Put(r.Context(), "error", errors.New("cannot get reservation from session"))
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	err := r.ParseForm()
	if err != nil {
		m.App.Session.Put(r.Context(), "error", errors.New("can't parse form"))
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	htmlMessage := fmt.Sprintf(`
      ...
      Dear %v, <br>
      This is a confirmation of your reservation of %v from %v to %v.
    `,
		reservation.FirstName,
		reservation.Room.RoomName,
		reservation.StartDate.Format(dateFormat),
		reservation.EndDate.Format(dateFormat),
	)
	msg := models.MailData{
	...
	htmlMessage = fmt.Sprintf(`
      ...
      A reservation has been made for %v from %v to %v.
    `,
		reservation.Room.RoomName,
		reservation.StartDate.Format(dateFormat),
		reservation.EndDate.Format(dateFormat),
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	// We handle all aspects of the json right here so no need for error checking
	out, _ := json.MarshalIndent(resp, "", "     ")
	w.Header().Set("Content-Type", "application/json")
	_, _ = w.Write(out)
...
// AdminDashboard shows the admin tool dashboard
...
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	_ = render.Template(w, r, "admin-reservations-show.page.tmpl", &models.TemplateData{
		StringMap: stringMap,
		Data:      data,
		Form:      forms.New(nil),
	})
...
// AdminPostShowReservation handles posts with a changed reservation in the admin tool
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-%v", src), http.StatusSeeOther)
}
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				...
				for d := y.StartDate; d.After(y.EndDate) == false; d = d.AddDate(0, 0, 1) {
					reservationMap[d.Format(dateLayout)] = y.ReservationID
				}
			} else {
				// it,s a block
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		// this is a block we need to remove
	...
	for name := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			...
			roomID, _ := strconv.Atoi(exploded[len(exploded)-2])
			date := exploded[len(exploded)-1]
			log.Printf("Would insert block for room id: %d for date: %s", roomID, date)
		}
	}
