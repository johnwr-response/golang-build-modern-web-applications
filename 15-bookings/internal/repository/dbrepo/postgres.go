...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	query := `UPDATE users SET first_name = $1, last_name = $2, email = $3, access_level = $4, updated_at = $5 WHERE id = $1`
...
// DeleteReservation updates a reservation in the database
...
func (m *postgresDBRepo) AllRooms() ([]models.Room, error) {
	...
	query := `
		SELECT
			r.id, r.room_name, created_at, updated_at
		FROM
			rooms r
		ORDER BY
		    r.room_name
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	query := `
		SELECT
			rr.id, coalesce(rr.reservation_id, 0) AS reservation_id, rr.restriction_id, rr.room_id, rr.start_date, rr.end_date, rr.created_at, rr.updated_at
		FROM
